...
// Define rate limits based on subscription tier (same as before)
const rateLimits = {
  free: {
    limit: 100,
    window: 3600, // 100 requests per hour (3600 seconds)
...
const rateLimiter = new RateLimiter('scanpro:ratelimit');
...
export async function apiMiddleware(request: NextRequest) {
  ...
  const { success, remaining, reset } = await rateLimiter.limit(apiKey, limit, window);
