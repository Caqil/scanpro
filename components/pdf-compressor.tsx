...
export function MultiPdfCompressor() {
      ...
      const response = await fetch('/api/compress/download-zip', {
        ...
        body: JSON.stringify({ 
          files: completedFiles.map(f => ({
            filename: f.filename,
            originalName: f.originalName || f.filename.replace(/^.+-compressed\.pdf$/, f.fileUrl.split('/').pop() || 'compressed.pdf')
          }))
        }),
      });
              ...
              <FormField control={form.control} name="processAllTogether" render={({ field }) => (
                <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                  ...
                  <div className="space-y-1 leading-none">
                    ...
                    <p className="text-sm text-muted-foreground">{t('compressPdf.processing.processAllTogether')}</p>
                  </div>
                </FormItem>
              )} />
            </div>
  ...
  {t('fileUploader.dropHereDesc')} {t('fileUploader.maxSize')}
            ...
            {files.length > 0 && (
              <div className="border rounded-lg">
                <div className="p-3 border-b bg-muted/30 flex justify-between items-center">
                  <h3 className="font-medium">{t('compressPdf.filesToCompress')} ({files.length})</h3>
                    ...
                    <div key={fileItem.file.name} className="p-3 flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs text-muted-foreground">{formatFileSize(fileItem.file.size)}{compressedFiles[fileItem.file.name] && <span className="ml-2 text-green-600 dark:text-green-400">→ {formatFileSize(compressedFiles[fileItem.file.name].compressedSize)} ({compressedFiles[fileItem.file.name].compressionRatio} {t('compressPdf.reduction')})</span>}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        {fileItem.status === 'completed' && (
                          <div className="flex items-center gap-2">
                            <Badge variant="outline" className="bg-green-50 text-green-600 dark:bg-green-900/30 dark:text-green-400"><CheckCircledIcon className="h-3 w-3 mr-1" /> {t('compressPdf.status.completed')}</Badge>
                            <Button type="button" variant="ghost" size="sm" asChild className="text-sm">
                              <a href={compressedFiles[fileItem.file.name].fileUrl} download target="_blank"><DownloadIcon className="h-3.5 w-3.5 mr-1" /> {t('ui.download')}</a>
                            </Button>
                          </div>
                        )}
                        {fileItem.status === 'error' && (
                          <div className="flex items-center gap-2">
                            <Badge variant="outline" className="bg-red-50 text-red-600 dark:bg-red-900/30 dark:text-red-400"><CrossCircledIcon className="h-3 w-3 mr-1" /> {t('compressPdf.status.failed')}</Badge>
          ...
          <CardFooter className="flex justify-end">
            <Button type="submit" disabled={files.length === 0 || isProcessing} className="min-w-32">{isProcessing ? t('ui.processing') : t('compressPdf.compressAll')}</Button>
          </CardFooter>
        </Card>
        {error && <Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertDescription>{error}</AlertDescription></Alert>}
        {isProcessing && files.length > 1 && (
          <div className="space-y-2">
            <div className="flex items-center justify-between"><span className="text-sm font-medium">{t('compressPdf.overallProgress')}</span><span className="text-sm text-muted-foreground">{Object.values(progress).filter(p => p === 100).length} {t('compressPdf.of')} {files.length} {t('compressPdf.files')}</span></div>
        ...
        {totalStats && files.length > 1 && (allFilesProcessed || anyFilesFailed) && (
          <Card className="border-green-200 dark:border-green-900">
            <CardHeader className="pb-2">
              <CardTitle className="text-green-600 dark:text-green-400 flex items-center gap-2"><CheckCircledIcon className="h-5 w-5" /> {t('compressPdf.results.title')}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="text-center"><p className="text-sm text-muted-foreground">{t('compressPdf.results.totalOriginal')}</p><p className="text-lg font-semibold">{formatFileSize(totalStats.totalOriginalSize)}</p></div>
                <div className="text-center"><p className="text-sm text-muted-foreground">{t('compressPdf.results.totalCompressed')}</p><p className="text-lg font-semibold">{formatFileSize(totalStats.totalCompressedSize)}</p></div>
                <div className="text-center"><p className="text-sm text-muted-foreground">{t('compressPdf.results.spaceSaved')}</p><p className="text-lg font-semibold text-green-600 dark:text-green-400">{formatFileSize(totalStats.totalSaved)}</p></div>
                <div className="text-center"><p className="text-sm text-muted-foreground">{t('compressPdf.results.averageReduction')}</p><p className="text-lg font-semibold text-green-600 dark:text-green-400">{totalStats.compressionRatio}%</p></div>
              </div>
              {allFilesProcessed && files.length > 1 && (
                <Button className="w-full" type="button" variant="outline" onClick={handleDownloadAllAsZip}><DownloadIcon className="h-4 w-4 mr-2" /> {t('compressPdf.results.downloadAll')}</Button>
              )}
            </CardContent>
            <CardFooter className="text-xs text-muted-foreground">{t('fileUploader.filesSecurity')}</CardFooter>
          </Card>
        )}
      </form>
    </Form>
