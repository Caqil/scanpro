...
export function PdfRotator() {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile.type !== "application/pdf") {
      toast.error(t("rotatePdf.errors.invalidType") || "Please select a valid PDF file");
      return;
    }
  ...
  const handleRotatePage = (pageNumber: number, direction: "clockwise" | "counterclockwise") => {
  ...
  const handleRotateSelected = (direction: "clockwise" | "counterclockwise") => {
    if (selectedPages.length === 0) {
      toast.error(t("rotatePdf.errors.noSelection") || "Please select at least one page to rotate");
      return;
    }
  ...
  const handleProcessPdf = async () => {
    ...
    if (pageRotations.every(rotation => rotation.angle === 0)) {
      toast.error(t("rotatePdf.errors.noRotation") || "Please rotate at least one page before processing");
      return;
    }
    ...
    try {
      // Prepare the form data with the file and rotation instructions
  ...
  const handleDownload = () => {
    if (processedFileUrl) {
      ...
      link.download = file ? file.name.replace(".pdf", "_rotated.pdf") : "rotated.pdf";
  ...
  const handlePageFormatChange = (format: string) => {
    ...
    if (format === "all") {
      ...
      setSelectedPages(Array.from({ length: numPages }, (_, i) => i + 1).filter(p => p % 2 === 0));
    } else if (format === "odd") {
      setSelectedPages(Array.from({ length: numPages }, (_, i) => i + 1).filter(p => p % 2 === 1));
  ...
  const parseCustomRange = () => {
    ...
    for (const range of ranges) {
      if (range.includes("-")) {
        ...
        if (!isNaN(page) && page > 0 && page <= numPages && !newSelectedPages.includes(page)) {
  ...
  const renderUploadSection = () => (
    ...
    >
      ...
      <p className="mt-6 text-sm text-muted-foreground">
        {t("ui.filesSecurity") || "Your files are processed securely. All uploads are automatically deleted after processing."}
      </p>
    </div>
  );
